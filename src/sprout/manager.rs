...
use crate::{kubernetes::manager::{KubeResourceManager, ResourceManager}, sprout::kind::{kind_of, AsSproutKind, SproutKind}, grower::{grow_sprouts, delete_sprouts}};
...
impl SproutManager {
    ...
    where
        K: kube::Resource<Scope = NamespaceResourceScope> + Clone + serde::de::DeserializeOwned + serde::Serialize + Sync + Send + std::fmt::Debug + 'static + AsSproutKind,
    ...
    where
        ...
        <K as kube::Resource>::DynamicType: Default,
    {
        info!("Deleting sprouts for '{}/{}'", resource.namespace().unwrap_or_default(), resource.name_any());
    ...
    pub async fn new_namespace(&self, namespace: &str) -> Result<()> {
        ...
        for seed in lock.iter() {
            ...
            info!("Growing sprout of {} '{}/{}' to '{}/{}", seed.resource_type, seed.namespace, seed.name, namespace, seed.name);
            match seed.resource_type {
                SproutKind::ConfigMap => {
                    let cm_api: Api<ConfigMap> = Api::namespaced(self.client.clone(), &seed.namespace);
                ...
                SproutKind::Secret => {
                    let sec_api: Api<Secret> = Api::namespaced(self.client.clone(), &seed.namespace);
